...
**Database schema & migrations**: the schema is defined by ordered SQL files in `api/migrations/` (`0001_initial_schema.sql`, `0002_recurrence_end.sql`, …), applied on API startup by `api/src/migrate.js` — a minimal raw-SQL runner (no dependency) that records applied files in a `schema_migrations` table, wraps each in a transaction, and serializes concurrent starts with a Postgres advisory lock. To add a schema change, drop a new `NNNN_name.sql` file in `api/migrations/`. **Backward compatibility**: if it finds an existing `transaction` table but no `schema_migrations` rows (a database created by the pre-migrations `init.sql`), it baselines `0001` as applied rather than re-running it. Demo data (`api/demo-seed.sql`) is seeded by `maybeSeedDemo()` only when `DEMO_DATA=true` and `balance` is empty. There are five tables — `category`, `project`, `balance`, `transaction`, and `recurrence_exception`. `transaction` is the central table with `is_income BOOLEAN` (TRUE = invoice/income, FALSE = bill/expense), `counterparty`, `category` FK (expenses only), `project_id` FK, `paid BOOLEAN`, `recurrence TEXT` (NULL or `'weekly'`/`'monthly'`/`'quarterly'`/`'annually'`), and `recurrence_end DATE` (optional; recurrence stops after it). Money columns (`amount`, `balance_amount`) are `NUMERIC(12,2)` (migration `0003` — never store money as REAL; note the pg driver returns NUMERIC as **strings**, so the web always coerces with `Number(...)`). `recurrence_exception` (`0004`) records single occurrences of a recurring transaction the user marked handled (paid/skipped): `(transaction_id, occurrence_date)`, cascade-deleted with the transaction. `balance` holds point-in-time snapshots; the latest entry is used as the projection start date, which the dashboard API returns as `start_date` so the UI can warn when it's stale (>7 days).

**Recurrence behaviour**: recurring transactions (any non-null `recurrence`) are always projected forward regardless of `paid` status — the paid toggle is hidden for them in the UI. Non-recurring paid transactions are excluded from the dashboard projection. **Per-occurrence handling**: instead of `paid`, individual occurrences of a recurring item can be marked handled via `POST /api/transactions/:id/exceptions {occurrence_date}` (undo: `DELETE .../exceptions/:date`). Excepted occurrences are excluded from the projection and skipped when computing the listing's `next_due_date`; the Transactions UI exposes this as a "✓ <next date>" button (and an undo arrow when exceptions exist).
