...
function pivot(rows, labelKey, weekCount, allWeekEnds) {
  ...
  const grandTotal = weeks.reduce(
    (s, w) => s + names.reduce((ss, n) => ss + (lookup[`${w}__${n}`] || 0), 0),
    0
  );
...
function PivotTable({ title, rows, labelKey, weekCount, allWeekEnds }) {
  ...
  const { names, weeks, lookup, weekEnds, rowTotals, grandTotal, colTotals } = pivot(
    rows,
    labelKey,
    weekCount,
    allWeekEnds
        ...
        <table className="text-xs w-full">
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              {weeks.map((w) => (
                <th key={w} className="px-3 py-2 text-right font-medium text-gray-600 min-w-24">
                  {weekEnds[w] || `Wk ${w}`}
                </th>
              ))}
      ...
      <div className="overflow-x-auto">
        <table className="text-xs w-full">
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              {balances.map((b) => (
                <th
                  key={b.week_number}
                  className="px-3 py-2 text-right font-medium text-gray-600 min-w-24"
                >
                  {fmt(b.week_end)}
                </th>
              ))}
          ...
          <tbody className="divide-y divide-gray-100">
            {rows.map(({ label, key }) => (
              <tr key={key}>
                <td className="px-3 py-2 sticky left-0 bg-white font-medium text-gray-700">
                  {label}
                </td>
                {balances.map((b) => {
    ...
    return (
      <div className="max-w-md mx-auto text-center py-16">
        ...
        <p className="text-gray-500 mb-6">
          The 13-week projection starts from your latest balance. Add your current bank balance to
          begin forecasting.
        </p>
        <Link
          to="/balance"
          className="inline-block bg-blue-900 text-white rounded px-4 py-2 text-sm font-medium hover:bg-blue-800"
        >
          Add a balance
        </Link>
      </div>
